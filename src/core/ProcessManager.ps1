# Process Manager Pro - Version Modular MEJORADA v2 OPTIMIZADA
# Con sistema de debounce para prevenir cuelgues
...
$script:TreeViewUpdating = $false
$script:TreeViewScrollTimer = $null
$script:TreeViewEventsPaused = $false
$script:LastTreeViewUpdate = [DateTime]::MinValue
$script:TreeViewLayoutSuspended = $false
$script:StatsUpdateTimer = $null
...
$script:DebounceDelay = 300  # milliseconds
...
# Funcion de debounce generico
function Invoke-Debounce {
    param(
        [scriptblock]$Action,
        [int]$DelayMs = 300,
        [ref]$Timer
    ...
    $Timer.Value = New-Object System.Windows.Forms.Timer
    $Timer.Value.Interval = $DelayMs
    $Timer.Value.Add_Tick({
        $this.Stop()
        & $Action
        $this.Dispose()
    })
...
function Show-ToastNotification {
    ...
    # Usar runspace para no bloquear UI
    ...
    $powershell.AddArgument($Title)
    $powershell.AddArgument($Message)
    $powershell.AddArgument($Icon)
...
# Funcion para aplicar tema con debounce
function Apply-Theme {
    param($Control, $IsDarkMode)
    
    # Si ya está actualizando, marcar para actualización pendiente
    if ($script:TreeViewUpdating -and $Control -is [System.Windows.Forms.TreeView]) {
        return
    }
...
# Funcion para actualizar contadores de procesos con debounce
function Update-ProcessCounters {
    ...
    $timeSinceLastUpdate = ($currentTime - $script:LastUpdateTime).TotalMilliseconds
    
    if ($timeSinceLastUpdate -lt $script:DebounceDelay) {
        $script:UpdatePending = $true
        return
    }
    ...
    try {
        ...
        foreach ($categoryName in $ProcessConfig.Categories.Keys) {
            ...
            foreach ($node in $ProcessTreeView.Nodes) {
                if ($node.Text -like "$categoryName*") {
                    ...
                    # Usar BeginInvoke para actualización asíncrona
                    $ProcessTreeView.BeginInvoke([Action]{
        ...
        # Si hay actualización pendiente, ejecutarla después de un delay
        if ($script:UpdatePending) {
            $script:UpdatePending = $false
            Start-Sleep -Milliseconds $script:DebounceDelay
...
# Funcion para filtrar procesos con debounce
function Filter-ProcessTree {
    ...
    $script:SearchDebounceTimer.Interval = 250  # 250ms de delay
    $script:SearchDebounceTimer.Add_Tick({
        $this.Stop()
        
        # Aplicar el filtro real
        $script:TreeViewUpdating = $true
        $ProcessTreeView.BeginUpdate()
        
        try {
            $showOnlyFavorites = $ShowFavoritesCheckBox.Checked
            
            foreach ($categoryNode in $ProcessTreeView.Nodes) {
                $categoryVisible = $false
                
                foreach ($processNode in $categoryNode.Nodes) {
                    $matchesFilter = $FilterText -eq "" -or 
                                   $processNode.Text -like "*$FilterText*" -or 
                                   ($processNode.Tag -and $processNode.Tag.Name -like "*$FilterText*")
                    
                    $isFavorite = $processNode.Tag -and ($processNode.Tag.Name -in $script:FavoriteProcesses)
                    
                    $shouldShow = $matchesFilter -and (-not $showOnlyFavorites -or $isFavorite)
                    
                    if ($shouldShow) {
                        $processNode.ForeColor = if ($isFavorite) { 
                            [System.Drawing.Color]::Gold 
                        } else { 
                            if ($IsDarkMode) { $DarkModeColors.Foreground } else { $LightModeColors.Foreground }
                        }
                ...
                if ($categoryVisible -and ($FilterText -ne "" -or $showOnlyFavorites)) {
                    $categoryNode.Expand()
                }
            }
        }
...
# Función de filtrado con debounce para búsqueda
function Filter-ProcessTree {
    param($FilterText)
    
    if ($script:SearchTimer -ne $null) {
    ...
    $script:SearchTimer.Interval = 250  # 250ms delay
    $script:SearchTimer.Add_Tick({
        $script:SearchTimer.Stop()
        
        $script:TreeViewUpdating = $true
        $script:TreeViewEventsPaused = $true
        
        try {
            # Usar BeginUpdate para mejor rendimiento
            $ProcessTreeView.BeginUpdate()
            
            $FilterText = $script:LastSearchText
            $showOnlyFavorites = $ShowFavoritesCheckBox.Checked
            ...
            foreach ($categoryNode in $ProcessTreeView.Nodes) {
                ...
                foreach ($processNode in $categoryNode.Nodes) {
                    $process = $processNode.Tag
                    $isVisible = $true
                    
                    # Filtrar por texto
                    if ($FilterText -ne "") {
                        $isVisible = $process.Name -like "*$FilterText*"
                    }
                    
                    # Filtrar por favoritos
                    if ($showOnlyFavorites -and $isVisible) {
                        $isVisible = $process.Name -in $script:FavoriteProcesses
                    }
                    
                    # Solo cambiar visibilidad si es necesario
                    if ($processNode.Visible -ne $isVisible) {
                        $processNode.Visible = $isVisible
                    }
                    
                    if ($isVisible) { $hasVisibleChild = $true }
                }
                
                # Solo cambiar visibilidad de categoría si es necesario
                if ($categoryNode.Visible -ne $hasVisibleChild) {
                    $categoryNode.Visible = $hasVisibleChild
                }
            }
        } finally {
            $ProcessTreeView.EndUpdate()
    ...
    $script:LastSearchText = $FilterText
...
# Optimizaciones de rendimiento para el TreeView
# Habilitar doble buffer para reducir parpadeo
$treeViewType = $ProcessTreeView.GetType()
$propInfo = $treeViewType.GetProperty("DoubleBuffered", [System.Reflection.BindingFlags]::NonPublic -bor [System.Reflection.BindingFlags]::Instance)
if ($propInfo) {
    $propInfo.SetValue($ProcessTreeView, $true, $null)
}

# Configurar propiedades adicionales para mejor rendimiento
$ProcessTreeView.BeginUpdate()
$ProcessTreeView.DrawMode = [System.Windows.Forms.TreeViewDrawMode]::OwnerDrawText
$ProcessTreeView.Add_DrawNode({
    param($sender, $e)
    
    # Solo dibujar si el nodo es visible
    if ($e.Node.IsVisible) {
        $e.DrawDefault = $true
    }
})
...
# Funcion actualizar estadisticas con debounce y optimización
function Update-Stats {
    # Usar BeginInvoke para actualización asíncrona
    if ($SelectedListView.InvokeRequired) {
        $SelectedListView.BeginInvoke([Action]{
            Update-StatsInternal
        })
    } else {
        Update-StatsInternal
    }
...
# Función interna para actualizar estadísticas
function Update-StatsInternal {
    try {
        ...
        # Usar BeginUpdate/EndUpdate para mejor rendimiento
        ...
        if ($selectedCount -gt 0) {
            ...
            $SelectedListView.Items.AddRange($items)
        }
        
        $SelectedListView.EndUpdate()
    } catch {
        Write-Host "Error actualizando estadísticas: $_" -ForegroundColor Red
    }
}

# Poblar TreeView con optimizaciones
$ProcessTreeView.BeginUpdate()
$ProcessTreeView.SuspendLayout()
...
try {
    foreach ($categoryName in $ProcessConfig.Categories.Keys) {
        ...
        if ($processes.Count -gt 0) {
            ...
            # Crear nodos en lote para mejor rendimiento
            $processNodes = @()
            ...
            # Agregar todos los nodos de proceso de una vez
            $categoryNode.Nodes.AddRange($processNodes)
            $ProcessTreeView.Nodes.Add($categoryNode) | Out-Null
        }
    }
} finally {
    $ProcessTreeView.ResumeLayout($false)
    $ProcessTreeView.EndUpdate()
}

# Función para actualizar stats con debounce mejorado
function Update-StatsDebounced {
    # Prevenir actualizaciones muy frecuentes
    $now = [DateTime]::Now
    $timeSinceLastUpdate = ($now - $script:LastTreeViewUpdate).TotalMilliseconds
    
    if ($timeSinceLastUpdate -lt 100) {
        # Si la última actualización fue hace menos de 100ms, programar para después
        if ($script:TreeViewScrollTimer -eq $null) {
            $script:TreeViewScrollTimer = New-Object System.Windows.Forms.Timer
            $script:TreeViewScrollTimer.Interval = 150
            $script:TreeViewScrollTimer.Add_Tick({
                $script:TreeViewScrollTimer.Stop()
                $script:TreeViewScrollTimer.Dispose()
                $script:TreeViewScrollTimer = $null
                Update-Stats
                $script:LastTreeViewUpdate = [DateTime]::Now
            })
            $script:TreeViewScrollTimer.Start()
        }
    } else {
        Update-Stats
        $script:LastTreeViewUpdate = $now
    }
}

# Evento TreeView con debounce mejorado
$ProcessTreeView.Add_AfterCheck({
    param($sender, $e)
    
    if ($script:TreeViewUpdating -or $script:TreeViewEventsPaused) {
        return
    }
    
    # Procesar cambios sin actualizar UI inmediatamente
    if ($e.Node.Tag -ne $null) {
        ...
        # Pausar eventos mientras actualizamos nodos hijos
        $script:TreeViewEventsPaused = $true
    ...
    Update-StatsDebounced
})

# Agregar manejador de scroll para el TreeView con optimizaciones
$ProcessTreeView.Add_MouseWheel({
    param($sender, $e)
    
    # Pausar temporalmente las actualizaciones durante el scroll
    $script:TreeViewEventsPaused = $true
    
    # Suspender layout si no está ya suspendido
    if (-not $script:TreeViewLayoutSuspended) {
        $script:TreeViewLayoutSuspended = $true
        $ProcessTreeView.SuspendLayout()
    }
    
    # Cancelar timer anterior si existe
    if ($script:TreeViewScrollTimer -ne $null) {
        $script:TreeViewScrollTimer.Stop()
        $script:TreeViewScrollTimer.Dispose()
    }
    
    # Crear nuevo timer para reanudar después del scroll
    $script:TreeViewScrollTimer = New-Object System.Windows.Forms.Timer
    $script:TreeViewScrollTimer.Interval = 150  # Reducido de 200ms para mejor respuesta
    $script:TreeViewScrollTimer.Add_Tick({
        $script:TreeViewScrollTimer.Stop()
        $script:TreeViewScrollTimer.Dispose()
        $script:TreeViewScrollTimer = $null
        
        # Reanudar layout y eventos
        if ($script:TreeViewLayoutSuspended) {
            $ProcessTreeView.ResumeLayout($true)
            $script:TreeViewLayoutSuspended = $false
        }
...
# Variable para controlar el estado del layout
$script:TreeViewLayoutSuspended = $false
...
$CloseButton.Add_Click({
    ...
    if (Show-ImpactPreview -SelectedProcesses $SelectedProcesses) {
        ...
        Invoke-Debounce -Action { Update-ProcessCounters } -DelayMs 500 -Timer ([ref]$null)
    }
})
...
$FavoritesButton.Add_Click({
    ...
    # Actualizar visual con debounce
    $script:TreeViewUpdating = $true
    $ProcessTreeView.BeginUpdate()
...
# Configurar timer para actualización automática con debounce mejorado
$script:RefreshTimer = New-Object System.Windows.Forms.Timer
$script:RefreshTimer.Interval = 10000  # 10 segundos (aumentado de 5)
$script:RefreshTimer.Add_Tick({
    # Solo actualizar si no hay actualizaciones pendientes
    if (-not $script:IsUpdating -and -not $script:UpdatePending) {
        Update-ProcessCounters
    }
})
...
$Form.Add_Shown({
    ...
    # Iniciar timer y actualizar contadores con delay inicial
    Start-Sleep -Milliseconds 500
    Update-ProcessCounters
    $script:RefreshTimer.Start()
    
    # Mostrar notificacion de bienvenida
    Show-ToastNotification -Title "Process Manager Pro v2" -Message "Sistema optimizado con debounce iniciado!" -Icon "Info"
})
